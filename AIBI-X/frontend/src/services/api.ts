...
export interface DeviceStatus {
  ...
  policy_score: number;
  drift_score: number;
  digital_twin_deviation: number;
  slow_poison_score: number;
  recon_score: number;
  impersonation_score: number;
  adversarial_score: number;
  future_risk: number;
  predicted_trust_2min: number;
...
export const getTrendData = async (deviceId: string) => {
  const response = await axios.get(`${API_BASE}/api/trend/${deviceId}`);
  return response.data;
};
