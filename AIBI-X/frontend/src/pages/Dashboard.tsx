...
import { getDevices, getDeviceStatus, getTrendData, analyzeNow, getExplanation } from '../services/api';
  ...
  const fetchOverviewData = async () => {
    try {
      ...
      if (targetId) {
        ...
        // 1. Fetch Trend History for the Chart
        try {
          const trend = await getTrendData(targetId);
          if (trend.timestamps && trend.timestamps.length > 0) {
            const mappedTrend = trend.timestamps.map((ts: string, idx: number) => ({
              time: new Date(ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
              trust: trend.trust_scores[idx] || 0,
              drift: trend.drift_scores[idx] || 0,
              poison: trend.slow_poison_scores[idx] || 0
            }));
          ...
          {selectedDeviceId && selectedStatus ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                {/* Advanced Threat Modules (Master Upgrade) */}
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="glass-panel p-4 border-l-2 border-cyber-blue">
                    <p className="text-xs text-slate-400 mb-1">Digital Twin Drift</p>
                    <h3 className={`text-xl font-bold ${selectedStatus.digital_twin_deviation > 30 ? 'text-cyber-warning' : 'text-white'}`}>
                      {(selectedStatus.digital_twin_deviation || 0).toFixed(1)}%
                    </h3>
                  </div>
                  <div className="glass-panel p-4 border-l-2 border-slate-500">
                    <p className="text-xs text-slate-400 mb-1">Slow Poison Risk</p>
                    <h3 className={`text-xl font-bold ${selectedStatus.slow_poison_score > 30 ? 'text-cyber-warning' : 'text-white'}`}>
                      {(selectedStatus.slow_poison_score || 0).toFixed(1)}%
                    </h3>
                  </div>
                  <div className="glass-panel p-4 border-l-2 border-cyber-danger/50">
                    <p className="text-xs text-slate-400 mb-1">Future Risk Prediction</p>
                    <h3 className={`text-xl font-bold ${selectedStatus.future_risk > 30 ? 'text-cyber-danger' : 'text-white'}`}>
                      {(selectedStatus.future_risk || 0).toFixed(1)}%
                    </h3>
                  </div>
                  <div className="glass-panel p-4 border-l-2 border-emerald-500/50">
                    <p className="text-xs text-slate-400 mb-1">Projected Trust (2m)</p>
                    <h3 className={`text-xl font-bold ${(selectedStatus.predicted_trust_2min || 100) < 60 ? 'text-cyber-danger' : 'text-emerald-500'}`}>
                      {(selectedStatus.predicted_trust_2min || 100).toFixed(0)}
                    </h3>
                  </div>
                </div>
